...
developments = pd.read_sql_query('SELECT job_number, address_house, address_street, boro FROM developments WHERE address_house IS NOT NULL AND address_street IS NOT NULL AND address IS NOT NULL AND boro IS NOT NULL AND geo_bbl IS NULL;', engine)
